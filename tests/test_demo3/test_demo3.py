...
def test_demo3():
    ...
    def check_no_relo_reextr():
        x=read_sql(select(DBSTRUCT().get_trove_dbtables(DDEF().troves[''].name)['reload'])); assert len(x)==0
        x=read_sql(select(DBSTRUCT().get_trove_dbtables(DDEF().troves[''].name)['reextr'])); assert len(x)==0
